...
class Modern_Stage(nn.Module):
    def __init__(self, num_blocks: int, ffn_ratio: int, large_size: int, small_size: int, dmodel: int, nvars: int, drop: float) -> None:
...
class Model(nn.Module):
    ...
    def __init__(self, configs) -> None:
        ...
        self.n_vars = configs.enc_in  # 通常为 2 (I/Q)
        ...
        for i in range(num_stages - 1):
            down = nn.Sequential(
                ...
                nn.Conv1d(stg_dims[i], stg_dims[i+1], kernel_size=self.downsample_ratio, stride=self.downsample_ratio)
            )
        ...
        self.flatten_dim = self.n_vars * stg_dims[-1] * final_len

        if self.task_name == 'AMC':
            self.amc_classifier = self._build_classifier(self.flatten_dim, configs.n_classes_amc, configs.dropout)
            
        elif self.task_name == 'WTC':
            self.wtc_classifier = self._build_classifier(self.flatten_dim, configs.n_classes_wtc, configs.dropout)
            ...
            # 异常检测：投影回原始维度 (Batch, Vars, Length)
            self.ad_projection = nn.Linear(self.flatten_dim, configs.enc_in * self.seq_len)
    ...
    def feature_extraction(self, x: torch.FloatTensor) -> torch.FloatTensor:
        """
        公共特征提取流程
        x: [B, M, L]
        """
    ...
    def ad(self, x):
        ...
        return out.reshape(feat.shape[0], self.n_vars, self.seq_len) # 重建 [B, M, L]
