...
class AdaCorrModule(nn.Module):
    def __init__(self, N: int) -> None:
        ...
        if isinstance(N, (tuple, list)):
            ...
            n_scalar = int(N)
        self.Im = TinyMLP(n_scalar)
        self.Re = TinyMLP(n_scalar)
...
class Model(nn.Module):
    ...
    def __init__(
        ...
        if isinstance(self.sig_len, (tuple, list)):
            self.sig_len = int(self.sig_len[0])
        else:
        ...
        msm_out_channels = (self.extend_channel // 3) * 3

        self.conv_chan_list = getattr(configs, "conv_chan_list", None)
        if self.conv_chan_list is None:
            # 将第一个元素设为 msm_out_channels (即 126)
            self.conv_chan_list = [msm_out_channels, 64, 128, 256]
        else:
            self.conv_chan_list[0] = msm_out_channels
